...
// Each line's length and speed are re-rolled within these bounds per mount, so
// the field never plays out identically twice.
...
function FlowLine({ config, screenWidth, screenHeight }: { config: LineConfig; screenWidth: number; screenHeight: number }) {
    ...
    // Halo softens across the line's WIDTH (perpendicular to travel); core fades
    // along its LENGTH, weighted toward the leading edge for a comet-like streak.
...
// Thin light-streaks that sweep across the screen — the recurring "signal" motif
// used behind splash, onboarding, welcome, and every auth screen.
