...
export const usePlaybackStore = create<PlaybackState>((set, get) => {
    // Syncs track metadata and session progress with OS lock screen controls
    ...
    return {
        ...
        updateAxis(x, y) {
            ...
            for (const layer of STEM_LAYERS) {
                ...
                if (!voice) continue;
                voice.gain.gain.setTargetAtTime(gains[layer], now, GAIN_SMOOTH_SECONDS);
            }
            set({ axisValues: { x, y } });
        },
        ...
        setSessionMinutes(minutes, sessionMode = 'continuous', cycleRhythm = 'classic') {
            ...
            if (minutes === null) {
                set({ sessionMinutes: null, sessionEndAt: null, sessionTimer: null, pausedRemainingMs: null, sessionMode, cycleRhythm });
                updateNowPlaying();
                return;
            }
        ...
        pause() {
            const {
                ...
                sessionEndAt,
                sessionTimer,
                playSegmentStartedAt,
                sessionPlaySeconds,
                playbackStartedAt,
                pausedOffsetSeconds,
            } = get();
            if (!isPlaying || !context || !currentTrack) return;
        ...
        // Adjusts remaining session duration by delta seconds and updates timer
